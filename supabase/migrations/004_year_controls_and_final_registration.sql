...
-- A separate, service-role-only correction path for the admin dashboard.
-- It locks the target group before counting, so concurrent moves cannot exceed capacity.
...
begin
  ...
  if existing_registration.id is null then raise exception 'REGISTRATION_NOT_FOUND' using errcode = 'P0001'; end if;

  select * into target_group from public.groups
  where id = p_group_id and academic_year = p_academic_year for update;
  if not found then raise exception 'GROUP_NOT_FOUND' using errcode = 'P0001'; end if;

  if existing_registration.group_id <> p_group_id then
    select count(*) into current_count from public.registrations where group_id = p_group_id;
    if current_count >= target_group.max_capacity then raise exception 'GROUP_FULL' using errcode = 'P0001'; end if;
    update public.registrations set academic_year = p_academic_year, group_id = p_group_id where id = existing_registration.id;
    insert into public.registration_history (registration_number, event_type, from_group_id, to_group_id)
    values (trim(p_registration_number), 'moved', existing_registration.group_id, p_group_id);
  end if;
...
revoke all on function public.admin_move_registration(text, smallint, uuid) from public, anon, authenticated;
grant execute on function public.admin_move_registration(text, smallint, uuid) to service_role;
